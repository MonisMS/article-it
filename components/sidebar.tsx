...
export function Sidebar({ user, isAdmin, variant = "desktop", onRequestClose }: Props) {
  ...
  const navItemClass = (active: boolean) => {
    const base = collapsed
      ? "group relative flex h-11 items-center justify-center rounded-xl transition-all duration-200"
      : "group relative flex h-11 items-center gap-3 rounded-xl px-3 transition-all duration-200"

    return `${base} ${active
      ? "bg-[#FFF2DA] text-[#2C2417] shadow-[inset_0_0_0_1px_rgba(194,106,10,0.10)] dark:bg-app-surface-hover dark:text-app-text"
      : "text-[#6F675A] hover:bg-[#F3EEE4] hover:text-[#2F271D] dark:text-app-text-muted dark:hover:bg-app-surface-hover dark:hover:text-app-text"}`
  }
  ...
  const renderNavItem = ({ href, icon: Icon, label }: { href: string; icon: React.ComponentType<{ className?: string }>; label: string }) => {
    ...
    return (
      ...
      >
        <Icon className={`h-5 w-5 shrink-0 ${active ? "text-[#C26A0A] dark:text-app-accent" : "text-[#8A8173] group-hover:text-[#5B5245] dark:text-app-text-subtle dark:group-hover:text-app-text"}`} />
        {!collapsed && (
  ...
  return (
    ...
    >
      <div>
        <div
          className={`mb-6 h-11 ${collapsed ? "flex items-center justify-center" : "flex items-center justify-between px-3"}`}
        >
          {!collapsed ? (
            <>
        ...
        <div className="mt-6 border-t border-[#EAE4D8] pt-6 dark:border-[#2A3442]">
          {!collapsed && (
      ...
      <div className="mt-4 border-t border-[#EAE4D8] pt-4 dark:border-[#2A3442]">
        {!collapsed ? (
          <>
            <div className="relative" ref={userMenuRef}>
              ...
              <button
                ...
                className="mb-2 flex w-full items-center gap-3 rounded-xl bg-[#F2EDE2] px-3 py-2.5 text-left transition-colors duration-150 hover:bg-[#ECE5D6] dark:bg-[#1C2633] dark:hover:bg-[#223041]"
              >
                ...
                <span className="flex items-center gap-1.5">
                  ...
                  <ChevronUp className={`h-4 w-4 text-[#8A8173] transition-transform duration-200 dark:text-[#9EA9B8] ${userMenuOpen ? "rotate-0" : "rotate-180"}`} />
                </span>
              </button>
            </div>
          </>
          ...
          <div className="relative flex flex-col items-center gap-2" ref={userMenuRef}>
            {userMenuOpen && (
              <div className={`${menuPanelClass} w-55 left-1/2 -translate-x-1/2`} role="menu" aria-label="User menu">
                <Link href="/upgrade" className={menuItemClass} role="menuitem" onClick={() => setUserMenuOpen(false)}>
                  <CreditCard className="h-4 w-4 text-[#8A8173] dark:text-[#9EA9B8]" />
                  Billing
                </Link>
